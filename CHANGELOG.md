...
- **Adapt to the `0.1.7` message-content block union: a tool result is a message, not a content block.** Harness `0.1.7` promotes a tool result to a first-class `tool`-role message carrying `toolCallId` and an optional `isError`, and removed `tool-result` from the message-content block union, so the retired wrapper can no longer be expressed as a `ContentBlock` at all. Both serializers (`src/serialize.ts`, `src/openai-serialize.ts`) had two defects from that change. First, `block.type === 'tool-result'` became an impossible comparison (`TS2367`) that narrowed the block to `never` and broke every field access behind it. Second, and worse, `serializeMessages` had no `tool`-role arm: a first-class tool result fell through to the user-role path and reached the wire as a spurious `role: 'user'` message carrying neither `tool_name` (Ollama) nor `tool_call_id` (OpenAI), so a local tool turn could not be replayed. Both serializers now accept `RequestMessage` — the request-only, identity-free user turn `GenerateOptions.messages` admits beside durable messages, whose `id` and `source` are `never` — emit a proper `role: 'tool'` message for a first-class tool result, ignore the `developer` tool-bookkeeping role instead of emitting an empty user message, and read the retired wrapper through a wide runtime view (`src/legacy-blocks.ts`) so a log written before `0.1.7` keeps its tool turns. The plugin only ever consumes that wrapper; it never writes one.

### Changed

- Move the `@deepseek-ai/dsh-*` pins from `0.1.7-alpha.2` to `0.1.7-rc.1` and re-verify against that host line. `dshWorkshop.compatibility.dshVersions` records `0.1.7-rc.1` alongside the earlier lines, the five READMEs name `dsh-v0.1.7-rc.1`, and the compat workflow installs the `0.1.7-rc.1` host (`@deepseek-ai/dsh`, `dsh-base`, `dsh-headless`). The declared peer ranges and `engines.dsh` are deliberately **unchanged**: the existing four-clause union already admits `0.1.7-rc.1`, and the family convention keeps the declared range wider than the verified line. `0.1.7-rc.1` **does** carry one plugin-facing change over `0.1.7-alpha.2` — the message-content block union above — so the pin move and the source adaptation have to land together; the pin move on its own leaves `typecheck`, `typecheck:ci` and the suite red. The read-only fallback keeps the earlier lines genuinely supported: a `0.1.6` log's `tool-result` wrapper is still read, so the widened peer range stays truthful.
- CI runs the repo-tsconfig `pnpm run typecheck` beside `typecheck:ci`, so both rulers the READMEs and `AGENTS.md` name are enforced rather than merely documented. `tsconfig.checkout.json`, which `package.json` already declared through `typecheck:checkout`, is restored: the third ruler had no config file, so the declared script exited `TS5058` instead of type-checking anything. It stays out of CI by design — it maps `@deepseek-ai/*` to a sibling `deepseek-harness` checkout and is a developer-local ruler.
- The four translations carry the star-CTA section that only `README.md` had. `scripts/check-readme-sync.mjs` compares `## ` headings for exact equality, so an English-only section had been failing the five-language gate on `main`.
